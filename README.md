...
AWM's Bayesian beliefs were designed for agent pipelines, but the same predict→observe→update loop works on any sequential decision domain. The equity benchmark tests whether fusing Le-WM's latent time-series representations with AWM's regime beliefs produces a tradeable edge on real market data.
...
|-----------|-------------|
| **Le-WM** | Temporal conv encoder (JEPA-style). 60-day OHLCV → 64-dim latent embedding. Learns to predict next embedding, not reconstruct input. |
| **AWM** | Beta distribution posteriors per market regime (trending up/down, mean-reverting, quiet). Updated from outcomes. |
| **XGBoost** | Gradient-boosted trees on handcrafted features (RSI, MACD, Bollinger %B, rolling vol/returns, volume ratio). Baseline. |
| **Fusion** | 50/50 blend of Le-WM logistic regression + AWM regime beliefs. |

### Bear Market Stress Test Results (2005–2026)

77 walk-forward windows × 63 trading days each, covering GFC, COVID, and 2022 bear market.
...
**By regime — where it gets interesting:**
...
**Key finding:** XGBoost wins overall and in calm markets. But Fusion dominates in the 2022 bear market (0.598 vs 0.517) — exactly when you need it most. This validates the core thesis: Bayesian regime awareness adds the most value during regime transitions and crises.
...
# Run bear market stress test (requires OMP_NUM_THREADS=1 on Apple Silicon)
OMP_NUM_THREADS=1 python bear_market_stress_test.py --device cpu
...
- **PyTorch + XGBoost SIGSEGV on Apple Silicon**: Set `OMP_NUM_THREADS=1` before importing either library. They fight over OpenMP.
- **Zero trades at 0.55+ thresholds**: Well-calibrated models output probabilities near 0.50. Fixed thresholds above 0.55 produce zero trades. This is correct calibration behavior — v2 will use top-k selection instead.

### Specification

Formal requirements and design documents: [`openspec/`](./openspec/).
